DELETE FROM public.sheet_template_role_overrides stro
USING public.store_roles sr
WHERE stro.store_role_id = sr.id
...
BEGIN
  ...
  SELECT sm.role_id, sr.slug, COALESCE(sr.permissions, '{}'::jsonb)
  INTO current_role_id, current_role_slug, base_permissions
  ...
  IF lower(COALESCE(current_role_slug, '')) = 'owner' THEN
